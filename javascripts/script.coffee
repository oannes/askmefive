$ ->
    ...
    count = (sign = 1) ->
        _(watches).each((w) -> w.total += w.points[currentStatement] * sign)
        currentStatement += 1

    render = -> 
        if currentStatement < statements.length
            $("#container").fadeIn(-> $(this).removeClass("hide")) if($("#container").hasClass("hide"))
            $("#js-content").html(Handlebars.compile($("#step-template").html())(statement: statements[currentStatement]))
            $("#js-progress > .meter").animate(width: 100*currentStatement/statements.length + "%")
        else
            winner = _(watches).sortBy("total").reverse()[0]
            # second_best = _(watches).sortBy("total").reverse()[1]
            # worst = _(watches).sortBy("total").reverse()[9]
            resultHtml = Handlebars.compile($("#result-template").html())(winner)
            $("#js-progress > .meter").animate(width: "100%", { complete: -> $("#container").html(resultHtml) })

    $(document)
        .on("click", "#js-me", onMeClick)
        .on("click", "#js-notme", onNotMeClick)
        .on("click", "#js-share", onShareClick)
        .on("click", "#js-reload", onReloadClick)  
